...
classdef DiscriminativeRBM<RBM
    ...
    methods (Access=public)
        ...
        function train(obj,data)
            ...
            trainDataPatch=full(tools.ind2vec(data.trainLabels'+1)');
            ...
            for epoch=1:obj.rbmParams.maxEpoch
                ...
                if (perf <= 0.08 )%&& epoch >= 100)
                   break;
                elseif (perf <= 0.095 && epoch >= 400)
                  break;
                end                
        ...
        function [extractedFeature]=getFeature(obj,dataMatrix,k,fixedDimensions,dataMatrixLabels)
            ...
            if nargin<5 || isempty(dataMatrixLabels)
                ...
                dataMatrixLabelsVector=full(tools.ind2vec(dataMatrixLabels'+1,obj.rbmParams.numberOfVisibleSoftmax)');
            end
        ...
        function [generatedData,generatedLabel,classesSoft]=generateData(obj,extractedFeature,k)
            if nargin<3
                k=1;
            end
            ...
            % Retornando as classes com softmax
            classesSoft = visProb(:,1:obj.rbmParams.numberOfVisibleSoftmax);
            
            %Determine labels
            [~,I]=max(visProb(:,1:obj.rbmParams.numberOfVisibleSoftmax),[],2);
            generatedLabel=I-1;
            %Determine data
            generatedData=tools.gather(visSample(:,obj.rbmParams.numberOfVisibleSoftmax+1:end));
        ...
        function [reconstructedData,reconstructedLabel,classesSoft]=reconstructData(obj,dataMatrix,k,fixedDimensions,dataMatrixLabels)
            if nargin<3
                k=1;
            end
            if nargin<4
            ...
            [reconstructedData,reconstructedLabel,classesSoft]=obj.generateData(extractedFeature,1);
        ...
        function [classNumber,classesSoft]=predictClass(obj,dataMatrix,method)
            if nargin<3
                method='byFreeEnergy';
            end
            switch method
                ...
                case 'bySampling'
                    [~,classNumber,classesSoft]=obj.reconstructData(dataMatrix,1);
                ...
                case 'byFreeEnergy'
                    ...
                    for i=1:numclasses
                        ...
                        F(:,i)=tools.gather(SamplingClasses.freeEnergy(obj.rbmParams,dataMatrix));
                    end
                    %take the min
                    [~,I]= min(F, [], 2);
                    classNumber=I-1;
